...
def test_sanitize_snapshot_masks_path_fields():
    payload = {
        ...
        "items": [{"log_path": "D:/workspace/private/app.log"}],
    }
    ...
    assert masked["items"][0]["log_path"] == "<redacted>/app.log"
...
def test_build_diagnostic_bundle_writes_expected_entries(tmp_path: Path):
    ...
    with zipfile.ZipFile(output) as zf:
        ...
        assert "state/preview_rows.json" not in names
