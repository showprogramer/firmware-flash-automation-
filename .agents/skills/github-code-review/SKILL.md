...
- Authenticated with GitHub (`gh auth login` or `GITHUB_TOKEN` set)
...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review via automated review\",
    \"comments\": [
...
Set `GH_OWNER`, `GH_REPO`, and `GITHUB_TOKEN` from environment or `gh auth`.
