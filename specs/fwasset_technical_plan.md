...
**背景：** 当前 SQLite 只存扫描快照（只读），缺少员工可维护的业务数据层。

未来需要在软件内做增删改操作（而非在文件目录操作）时，需要区分两类数据：

| 类型 | 描述 | 当前状态 |
|---|---|---|
| 扫描快照 | 反映磁盘现状，由扫描引擎写入 | ✅ 已有（`assets` 表） |
| 业务标注 | 员工手动维护，标签/备注/验证状态等 | ❌ 缺失 |

### 行动项

- [ ] 设计 `asset_annotations` 表结构（关联 `asset.path`，字段包含：标签、备注、验证状态、操作人、时间戳）
- [ ] 新建 `src/fwasset/core/annotation_index.py` 提供 CRUD 接口
- [ ] UI 详情面板增加标注编辑区
- [ ] 扫描刷新时保留业务标注（不随扫描清空）

**预估工作量：** 表设计 0.5 天；CRUD 接口 1 天；UI 接入 1 天
...
| # | Issue | 优先级 | 建议时机 | 预估工时 |
|---|---|---|---|---|
| 2 | SQLite 查询替换内存过滤 | 🔴 高 | 立即 | 1 天 |
| 1 | FirmwareListPanel 拆分（ViewModel 层） | 🔴 高 | 下个迭代开始 | 3～5 天 |
| 5 | flash_mode 操作面板注册表 | 🟡 中 | 拆分 Issue 1 时同步做 | 2 天 |
| 7 | 业务标注数据层 | 🟡 中 | 有增删改需求时 | 2.5 天 |
| 4 | Settings 对象化 | 🟡 中 | 有配置界面需求时 | 2 天 |
| 3 | 增量扫描（mtime + 取消） | 🟡 中 | 出现慢扫描投诉时 | 1.5 天 |
| 6 | catalog 匹配可解释化 | 🟢 低 | 有误判投诉时 | 1.5 天 |
| 8 | 日志轮转与结构化 | 🟢 低 | 运维阶段 | 1 天 |
