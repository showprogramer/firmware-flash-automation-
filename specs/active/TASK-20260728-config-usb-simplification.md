...
| --- | --- |
| 类型 | 设计 + 拆解（多子任务） |
| 当前状态 | 📐 讨论中，待细化后立项各子 TASK |
| 前置 | Phase C 已完成；`feature/pyside6-migration` 已通过人验 |
| 父任务 | — |
| 分支 | `feature/pyside6-migration` 或新分支（待定） |
...
- [ ] `dist/` 目录只含 `fwasset.exe` + `firmware_catalog.toml`
- [ ] 新机器首次启动弹向导，配置完成后正常进入主界面
- [ ] 已有配置的机器升级后行为不变（迁移策略确认）
- [ ] 手控烧录流程：格式化成功 + 文件复制成功 = 任务完成
- [ ] `.\scripts\test.ps1` 通过，覆盖率 ≥ 80%
