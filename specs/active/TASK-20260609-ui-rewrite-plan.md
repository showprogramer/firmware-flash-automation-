...
1. `auto_usb_panel.py`：
   ...
   - U盘盘符来源：改用 `panel_host.get_global_usb_drive()`；若为空，点按钮时提示先选择目标 U 盘。
...
- **单变体折叠**：单变体模块直接一行（带版本），点一下即出操作，**不再 `主板程序/主板程序` 重复**；
  多变体才父子可展开。程序名称若 == 程序类型则显示「默认」。
- **「打开烧录工具」**：待增删改查（CRUD）完整后再推进精确"打开对应工具"，当前先占位。

**实施清单：**
1. [x] 顶部检索条：独立搜索框（分词）+ 型号平铺按钮条（常用平铺 + `更多型号` 收纳）；移除旧 `model_combo` 穷举下拉。（2026-07-02）
2. [x] 搜索分词：`query_assets` 改空格分词 AND（含型号字段）。
3. [x] 单变体折叠 + 变体名去冗余（`populate_tree` / `data_grid_panel`）。
4. [x] ttk 主题：行高 ~32、交替行底色、归属语义色。（2026-07-02；悬停高亮/自定义箭头暂不做，Tk 原生 Treeview 支持有限）
5. [x] 「来源」改为更易懂的「程序归属」：`定制专属` / `通用默认`。（2026-07-06）
6. [x] 底部「已选 XXX」详情条 + 紧凑操作区。（2026-07-02）
7. [x] 删除「方式」列：除手控 UI（非断码屏）一键烧录外，其余主要依赖烧录工具，表格不再重复展示烧录方式。（2026-07-06）
8. [x] 表格列名调整为 `程序类型 / 程序名称 / 版本 / 程序归属 / 程序文件`，单变体重复名显示 `默认`。（2026-07-06）
9. [x] 删除左侧栏底部「工具中心」按钮，底部仅保留「扫描目录」入口。（2026-07-06）
10. [x] 表头对齐方式跟随内容列：文本列左对齐，版本/归属状态列居中。（2026-07-06）

**视觉复盘补充（2026-07-06）：**
用户用 `specs/current_project_ui/ui.png` 对比 `specs/images/treeview_workbench_mockup.png` 后确认当前视觉仍明显偏丑。已做一轮 Fluent 桌面化修正：
- 固定左侧栏宽度，侧栏只承载导航，型号/搜索迁移到主区顶部筛选条。
- 移除侧栏字符画与 emoji 图标，改为低噪声导航按钮和选中态。
- 表格行文字从整行蓝色收敛为正常文本色，保留交替行底色；「方式」列经复盘后删除。
- 表格语言从「模块类型 / 变体名称」收敛为面向烧录场景的「程序类型 / 程序名称」，主固件文件列改为「程序文件」。
- 表头对齐方式跟随内容列，消除表头居中、正文左对齐的不一致。
- 左侧栏底部删除「工具中心」按钮，避免当前阶段暴露低频入口。
- 表格、操作区、日志区补充细边界，底部操作区与日志高度压缩，减少大块空白。
...
### 手动验收清单
- [x] 点定制 → 某方案：右侧出现整机 7 模块行。
- [x] 手控UI 有多变体时收成 1 行可展开，展开见各变体，**不在顶层铺平**。
- [x] 缺失模块行标 ⚫通用默认，方案自带的标 🔵定制专属；**全程无「回源」字样**。
- [x] 双击变体行 = 打开该目录；**无「打开目录」按钮**。
- [x] 选手控UI 变体 → 操作区只有一个「一键烧录」，无 U盘选择器、无分步按钮、无复制路径。
- [x] 选 tool_launch 变体 → 只有「打开烧录工具」。
- [x] 操作区高度收敛，不再「大的离谱」。
- [x] 点定制方案/通用类型/搜索切换均不崩溃。
