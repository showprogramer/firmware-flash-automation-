...
### fix(core,ui): 收敛默认模块键并优化工作台右键菜单（TASK-20260803）

- 工作台资产右键菜单改用 QFluent 圆角菜单和一致图标；默认、共享来源及目录操作按层级分组。已登记共享来源时提供“更换共享来源…”与“取消共享来源”。
- `快捷键`、`快捷按键`、`快捷键-旋钮` 统一归一为 catalog 规范键 `快捷键程序`；读取历史重复 defaults 时规范键优先，默认写入会移除同义旧键。
- 默认徽章、方案补齐与自动更新共享来源统一采用规范键，避免“快捷键”与“快捷键程序”各自指向不同变体时出现两个默认。

验证：右键菜单定向测试 3 passed；默认配置服务、默认徽章 / 方案补齐与自动更新共享来源测试 24 passed。Windows 人工验收于 2026-08-03 通过。
...
- 首次配置向导支持路径预填，并在根目录为空、目录不存在或工具根目录无效时阻止保存；跳过行为明确说明可从设置页继续配置。
- 未配置根目录的工作台显示「前往设置」非阻断提示；原「扫描目录」用户文案调整为「重新读取程序文件夹」，且该操作只读取已配置的根目录，切换位置只能在设置中完成。
- 不新增向导跳过标记，不改 SQLite 单工作区语义；完整 CRUD 与扫描入口的最终迁移不在本次范围。

验证：冻结 exe 人工验证于 2026-07-28 通过；`.\scripts\test.ps1` → 381 passed，coverage 94.33%。
