...
def test_build_scan_result_ok(monkeypatch: pytest.MonkeyPatch):
    monkeypatch.setattr("handcontrol.core.services.scan_service.find_handcontrol_folders", lambda root: [{"model": "L36", "version": "V1.0.0", "label": "x"}])
    monkeypatch.setattr("handcontrol.core.services.scan_service.load_excel_status", lambda p, s: {("L36", "V1.0.0"): "测试通过"})
